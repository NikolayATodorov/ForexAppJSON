...
type RestOf<T extends IExchangeRate | NewExchangeRate> = Omit<T, 'timestamp'> & {
...
export class ExchangeRateService {
  ...
  protected convertDateFromClient<T extends IExchangeRate | NewExchangeRate | PartialUpdateExchangeRate>(exchangeRate: T): RestOf<T> {
    return {
      ...exchangeRate,
      timestamp: exchangeRate.timestamp?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restExchangeRate: RestExchangeRate): IExchangeRate {
    return {
      ...restExchangeRate,
      timestamp: restExchangeRate.timestamp ? dayjs(restExchangeRate.timestamp) : undefined,
    };
  }
